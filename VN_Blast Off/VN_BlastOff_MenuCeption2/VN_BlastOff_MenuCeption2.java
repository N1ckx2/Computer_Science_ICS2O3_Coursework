/*Blast Off with Menu within Menu
Nicholas Vadivelu
ICS203
October 30th, 2014*/

import java.awt.*;
import hsa.Console;
import javazoom.jl.player.Player;
import java.io.FileInputStream;

public class VN_BlastOff_MenuCeption2
{
    static Console c;
    
    public static AudioPlayer bgm = new AudioPlayer("Rocket.mp3");
    
//*******************************While*********************************

    public static void While ()  throws InterruptedException 
    {
	bgm.play();
	int x = 10; //declare variable
	
	while (x >= 0) //as long as x is bigger than 0 it runs
	{
	    c.clear(); // clears screen
	    if (x == 0)
	    {
		c.println("Blast Off!!!!"); //prints blast off at 0
	    }
	    else
	    {
		c.println(x); //counts down
	    }
	    Thread.sleep(1000); //waits for 500 ms
	    x--; //subtracts 1 from x
	}
	Thread.sleep(2000);
	bgm.stop();
    }

//*******************************DoWhile*********************************

    public static void DoWhile ()  throws InterruptedException 
    {
	bgm.play();
	int x = 10;//declare variable
	
	do {
	    c.clear(); // clears screen
	    if (x == 0)
	    {
		c.println("Blast Off!!!!"); //prints blast off at 0
	    }
	    else
	    {
		c.println(x); //counts down
	    }
	    Thread.sleep(1000);//waits for 500 ms
	    x--;//subtracts 1 from x
	    
	} while(x >= 0); //as long as x is bigger than 0 it runs
	Thread.sleep(2000);
	bgm.stop();
    }
    
//*******************************While*********************************

    public static void For ()  throws InterruptedException 
    {
	bgm.play();
	for(int x = 10; x >= 0; x--) //declare variable, while greater than 0, subtracts 1 from x
	{
	    c.clear();
	    if (x == 0) 
	    {
		c.println("Blast Off!!!!"); //prints blast off at 0
	    }
	    else
	    {
		c.println(x); //counts down
	    }
	    Thread.sleep(500); //waits for 500 ms
	}
	Thread.sleep(2000);
	bgm.stop();
    }
    
//***********************************Switch Menu***************************
    public static void  Switch() throws InterruptedException 
    {
	int choice;

	do
	{
	    //Promting for choice
	    c.println("Blast off by Nicholas Vadivelu\n");
	    c.println ("Choose from the following menu\n");
	    c.println ("Enter 1, 2, or 3");
	    c.println ("1 - While");
	    c.println ("2 - Do While");
	    c.println ("3 - For");

	    c.println ();
	    c.println ("Enter 0 to go to the main menu");
	    choice = c.readInt();
	    c.clear();
	    switch (choice)
	    {
		case 1: While();
			break;
		case 2: DoWhile();
			break;
		case 3: For();
			break;
		case 0: break;
		default: c.println("Input a valid number, use a different number");
			 break;
	    }

	c.print ("\n\nPress enter to continue");
	c.getChar ();
	
	c.clear ();
	    //This will loop until user enters 0
	}

	while (choice != 0);
	c.println ("Press enter to go to the main menu");
    }

//***********************************If/ Else*********************************
public static void If() throws InterruptedException 
{
	int choice;

	do
	{
	    //Promting for choice
	    c.println("Blast off by Nicholas Vadivelu\n");
	    c.println ("Choose from the following menu\n");
	    c.println ("Enter 1, 2, or 3");
	    c.println ("1 - While");
	    c.println ("2 - Do While");
	    c.println ("3 - For");

	    c.println ();
	    c.println ("Enter 0 to go to the main menu");
	    choice = c.readInt();
	    c.clear();
	    if (choice == 1) While();
	    else if (choice == 2) DoWhile();
	    else if (choice == 3) For();

	c.print ("\n\nPress enter to continue");
	c.getChar ();
	
	c.clear ();
	    //This will loop until user enters 0
	}

	while (choice != 0);
	c.println ("Press enter to go to the main menu");
	// Place your program here.  'c' is the output console
    } // main method
//***********************************Menu Program Here*************************
    public static void main (String[] args) throws InterruptedException 
    {

	c = new Console ();

	int choice;

	do
	{
	    //Promting for choice
	    c.println("Blast off by Nicholas Vadivelu\n");
	    c.println ("Choose from the following menu\n");
	    c.println ("Enter 1, or 2");
	    c.println ("1 - Switch");
	    c.println ("2 - If/ Else");

	    c.println ();
	    c.println ("Enter 0 to exit");
	    choice = c.readInt();
	    c.clear();
	    switch (choice)
	    {
		case 1: Switch();
			break;
		case 2: If();
			break;
		case 0: break;
		default: c.println("Input a valid number, use a different number");
			 break;
	    }

	c.print ("\n\nPress enter to continue");
	c.getChar ();
	
	c.clear ();
	    //This will loop until user enters 0
	}

	while (choice != 0);
	c.println ("The program has terminated");
	// Place your program here.  'c' is the output console
    } // main method
} // Menu class

class AudioPlayer implements Runnable{
    private Player player;
    private Thread thread;
    private boolean loop = false;
    private String fileName;
    public AudioPlayer(String s){
	load(s);
    }
    public void load(String s){
	fileName = s;
	try{
	    player = new Player(new FileInputStream(s));
	    thread = new Thread(this);
	}
	catch(Exception e){
	    e.printStackTrace();
	}
    }
    public void play(){
	if(thread != null) thread = null;
	if(thread == null) load(fileName);
	thread.start();
    }
    public void loop(){
	loop = true;
	play();
    }
    public void stop(){
	loop = false;
	thread = null;
	player.close();
    }
    public void run(){
	do{
	    try{
		player.play();
	    }
	    catch(Exception e){
		e.printStackTrace();
	    }
	    if(loop) load(fileName);
	}while(loop);
    }
}
